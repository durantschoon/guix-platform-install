...
func main() {
	...
	if runtime.GOARCH == "arm64" {
		...
		fmt.Println("     Your Mac is ARM64 (aarch64), while this published Oracle VM is x86_64.")
		fmt.Println("     Guix binary substitutes ONLY match between identical architectures:")
		fmt.Println("       - If you want an x86_64 fleet (to share binaries with x86 laptops, PCs,")
		fmt.Println("         and this Oracle VM), run an x86_64 container on your Mac via Rosetta:")
		fmt.Println("           docker run --platform linux/amd64 ...")
		fmt.Println("       - If you ONLY want to sync between your Mac and Oracle (no x86 machines),")
		fmt.Println("         you can alternatively launch an Oracle Ampere A1 (ARM64) instance to")
		fmt.Println("         stay purely aarch64 across both machines.")
		fmt.Println("     (See docs/GIPS_MULTI_MACHINE_SYNC.md for full guidance)")
		fmt.Println()
	}
